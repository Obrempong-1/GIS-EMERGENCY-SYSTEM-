...
export default function MapComponent({ locations, routePath, activeLocation, onUserLocationUpdate, transportMode = 'drive' }: any) {
    ...
    const handleNavigate = async (destination: any) => {
        ...
        // Obrempong's Georeference Check haha
        ...
        try {
            ...
            alert("Failed to calculate route. Ensure backend is running at http://localhost:8000");
        } finally {
    ...
    // KNUST Bounds (South-West to North-East)
            ...
            >
                ...
                {locations.map((loc: any, idx: number) => {
                    ...
                    if (isSpecial) {
                        ...
                        if (loc.type === 'medical') icon = hospitalIcon;
                        else if (loc.type === 'police') icon = policeIcon;
                        else if (loc.type === 'fire_station') icon = fireIcon;
                        else if (loc.type === 'incident') icon = incidentIcon;
                        else if (loc.type === 'user') icon = userIcon;
                ...
                {activeLocation && <MapController lat={activeLocation.lat} lon={activeLocation.lon} />}
